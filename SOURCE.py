...
from scipy.integrate import solve_ivp # For solving y' = f(x,y)
import scipy.integrate as integrate # For integration
import scipy.interpolate as spi # For splines
import sympy as sym # For symbolic math
import numpy as np # For number stuff
import math # More number stuff
...
class CatSolution:
    ...
    def __str__(self):
        ...
        if self.x is not None and self.y is not None:
            print(f"\tEndpoint: ({self.x[-1]}, {self.y[-1]})")
...
def find_parameters(dens, xdist, ydist, length, guess_h=None, guess_dydx=None, thresh=None, max_attempts=500, debug=False, type=hanging):
    global prev_t, prev_Sum, prev_y
...
def find_loaded_catenary(dens, xdist, ydist, length, thresh=.01, max_attempts=500, debug=False):
    def scale_dens(s):
        return dens(s*xdist)
    scale_cat = find_parameters(scale_dens, 1, ydist/xdist, length/xdist, thresh=thresh/xdist, max_attempts=max_attempts, debug=debug, type=loaded)
    if scale_cat.status == 0:
        loopCount = scale_cat.message[scale_cat.message.find("after")+6:scale_cat.message.find("attempts")-1]
        scale_cat.message = f"Success. Catenary found with endpoint = ({xdist}±{thresh}, {ydist}±{thresh}) after {loopCount} attempts."
    return CatSolution(message = scale_cat.message,
...
def dens_from_eq(x, shape, xdist):
    ...
    print(f"Density: ({y2prime})/sqrt(1 + ({yprime})^2)")
